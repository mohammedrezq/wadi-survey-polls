...
function display_custom_post_type($atts)
{
    ...
    if (!isset($existedRow) && $multistep_survey != true) {
        ?>
	<form method="POST" class="survey_container" data-survey-id="<?php echo esc_attr($the_post_id); ?>" data-user-id="<?php echo esc_attr($the_current_user_id); ?>" data-post-type="<?php echo esc_attr(get_post_type($the_post_id)); ?>">
        ...
        if (!empty($survey_items)) :
                    foreach ($survey_items as $survey_item) {
                        ...
                        if ($survey_item['select_survey_question_type'] == 'multiple_choices') {
							...
							<div class="multiple_container" id="<?php echo esc_html($theQuestionCleaned); ?>">
								<div class="multiple_choices_question"><?php echo wp_kses_post($multiple_question); ?></div>
                            ...
                            foreach ($survey_item['multiple_answers'] as $multiple_text_answers) {
                                ...
                                $theAnswerCleanup = trim(preg_replace('/\s+/', '', $available_multiple_answers)); ?>
									<div class="custom-control custom-radio">
										<input type="checkbox" id="customCheckbox_<?php echo esc_html($available_multiple_answers); ?>" data-answer="<?php echo esc_html($available_multiple_answers); ?>" data-question="<?php echo esc_html(wp_strip_all_tags($multiple_question_cleanup)); ?>" class="custom-control-input">
										<label class="custom-control-label" for="customCheckbox_<?php echo esc_html($available_multiple_answers); ?>"><?php echo wp_kses_post($available_multiple_answers); ?></label>
									</div>
									<?php
                            } ?>
							</div>
		...
		<button type="submit" class="wadi_survey_submit"><?php echo esc_attr__('Submit', 'wadi-survey') ?></button>
	</form>
	...
	<?php
    } elseif (!isset($existedRow) && $multistep_survey == true) {
...
<input type="hidden" data-survey-finish-message='<?php echo wp_kses_post($survey_finish_message); ?>' data-redirect-time="<?php echo esc_attr($redirect_time); ?>" data-survey-already-taken-message='<?php echo wp_kses_post($survey_already_taken_message); ?>' class="redirect_url" data-redirect-url='<?php echo esc_attr($redirect_url); ?>' />
	<?php
         ...
         */
    } elseif ($allow_multiple_responses == true && $multistep_survey != true) {
        ?>
		<form method="POST" class="survey_container" data-survey-id="<?php echo esc_attr($the_post_id); ?>" data-user-id="<?php echo esc_attr($the_current_user_id); ?>" data-post-type="<?php echo esc_attr(get_post_type($the_post_id)); ?>">
		<div class="survey_questions_conatiner">
				<?php
...
<input type="hidden" data-survey-finish-message='<?php echo wp_kses_post($survey_finish_message); ?>' data-redirect-time="<?php echo esc_attr($redirect_time); ?>" data-survey-already-taken-message='<?php echo wp_kses_post($survey_already_taken_message); ?>' class="redirect_url" data-redirect-url='<?php echo esc_attr($redirect_url); ?>' />
	<?php
         ...
         * End of Multistep Survey Code for shortcode
         */
    } elseif (isset($existedRow) && $allow_multiple_responses == false) {
        ?>
		<p><?php echo wp_kses_post($survey_already_taken_message); ?></p>
<?php
    }
